...
${dc.showServiceDetail?`<div class="sec"><div class="sec-title">Servicios incluidos</div><table><thead><tr><th>Servicio</th>${dc.showProviders?'<th>Proveedor</th>':''}${dc.showProviderRef?'<th>Referencia</th>':''}${dc.showServicePrices?'<th style="text-align:right">Precio</th>':''}</tr></thead><tbody>${res.services.map(s=>{const prov=providers.find(p=>p.id===s.providerId);const svcLabel=SVC_TYPES.find(t=>t.v===s.type)?.l||s.type;const iataLabel=s.type==='vuelo'&&(s.originCode||s.destinationCode)?` (${s.originCode||'?'} → ${s.destinationCode||'?'})${s.flightNumber?' · '+s.flightNumber:''}`:s.description?' — '+s.description:'';return`<tr><td>${svcLabel}${iataLabel}</td>${dc.showProviders?`<td>${prov?.name||'—'}</td>`:''}${dc.showProviderRef?`<td>${s.providerFileNumber||'—'}</td>`:''}${dc.showServicePrices?`<td style="text-align:right">${fmt$(s.salePrice,cur)}</td>`:''}</tr>`;}).join('')}</tbody></table></div>`:''}
